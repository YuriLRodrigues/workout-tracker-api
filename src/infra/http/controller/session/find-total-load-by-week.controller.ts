...
import { FindTotalLoadByWeekUseCase } from '@root/domain/application/use-cases/session/find-total-load-by-week.use-case';
...
import { SwaggerFindTotalLoadByWeekDto } from '../../dto/session/find-total-load-by-week.dto';

@ApiTags('Session')
@Controller('session')
export class FindTotalLoadByWeekController {
  constructor(private readonly findTodayByWorkoutId: FindTotalLoadByWeekUseCase) {}

  @SwaggerFindTotalLoadByWeekDto()
  @Roles({
    roles: [UserRole.USER, UserRole.MANAGER, UserRole.PERSONAL],
    isAll: false,
  })
  @Get('/total-load-by-week')
  async handle(@CurrentUser() payload: UserPayload) {
    ...
    return {
      loadDiff: session.value.loadDiff,
