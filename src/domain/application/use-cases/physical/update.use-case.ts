...
@Injectable()
export class UpdatePhysicalUseCase {
  constructor(
  ...
  async execute({ age, goal, height, userId, weight, bodyFat, muscleMass }: Input): Promise<Output> {
    ...
    if (PhysicalStatsNotFound()) return left(new ResourceNotFoundError());

    physical.update({ age, bodyFat, goal, height, muscleMass, weight });

    await this.physicalRepository.update({ physical });
